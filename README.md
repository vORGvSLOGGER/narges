...
│     ├─ components/shared/   ← PanelSwitcher, ProtectedRoute
│     └─ panels/
│        ├─ customer/  (HomePage, ProductPage, CategoryPage, CartPage, OrderTracking, Loyalty, …)
...
### v0.8.0 — Vercel هو المستضيف الوحيد (`b80c3b8`) — **HEAD الحالي**
...
**الحالة:** تجربة العميل كاملة على القاعدة الحقيقية (تصفّح → سلة → دفع → حفظ في `orders` → تتبّع).
لوحات الموظفين تقرأ/تحدّث الطلبات الحقيقية. التصميم مطبّق على شاشات العميل الأساسية.

**معلّق:**
1. **حسابات موظفين تجريبية** — إنشاء `admin@narges.sa` / `cashier@narges.sa` / `delivery@narges.sa` في Supabase
   Auth ثم ضبط أدوارها (يتطلب اتصال أداة Supabase).
2. **ربط دومين `narges-sa.vercel.app`** بأحدث نشرة إنتاج (الرابط كان مثبّتاً على نسخة قديمة).
3. **شاشات تصميم إضافية** (السلة/الدفع، التتبّع، الولاء، تسجيل الدخول، لوحات الأدمن/الكاشير/المندوب) —
   يمكن إكمالها بنفس توكنات التصميم.
4. **`VITE_GEMINI_KEY`** يُضاف كمتغيّر سرّي في Vercel لتفعيل ميزة «إضافة منتج بالصورة» في الإنتاج.

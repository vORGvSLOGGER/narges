...
export default function ProcessOrderPage() {
  ...
  return (
    <div className="min-h-screen bg-narjis-bg print:bg-white">
      ...
      <div className="p-4 space-y-4 max-w-2xl mx-auto">
        ...
        <div className="card p-4 print:shadow-none print:border print:rounded-none">
          <div className="text-center mb-4 print:mb-6">
            <div className="w-16 h-16 bg-narjis-green rounded-2xl flex items-center justify-center mx-auto mb-2 print:hidden">
              <span className="text-white text-3xl font-bold">ن</span>
            </div>
            <h2 className="font-bold text-xl text-narjis-green">نرجس سوبرماركت</h2>
            <p className="text-sm text-narjis-text-secondary">فاتورة ضريبية</p>
          </div>

          <div className="grid grid-cols-2 gap-2 text-sm border-y py-3 my-3">
            <div>
              <p className="text-narjis-text-secondary text-xs">رقم الطلب</p>
              <p className="font-bold text-narjis-green">{order.id}</p>
            </div>
            <div className="text-left">
              <p className="text-narjis-text-secondary text-xs">التاريخ</p>
              <p className="font-medium text-xs">{formatDate(order.createdAt)}</p>
            </div>
            <div>
              <p className="text-narjis-text-secondary text-xs">العميل</p>
              <p className="font-medium">{order.customerName}</p>
            </div>
            <div className="text-left">
              <p className="text-narjis-text-secondary text-xs">الدفع</p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {order.items.map((item, i) => (
                <tr key={i}>
                  <td className="py-2 text-narjis-text">{item.nameAr}</td>
                  <td className="py-2 text-center text-narjis-text-secondary">{item.qty}</td>
          ...
          {order.notes && (
            <div className="mt-3 bg-yellow-50 rounded-xl p-3 text-sm">
              ...
              <span className="text-narjis-text-secondary">{order.notes}</span>
            </div>
          )}
        </div>

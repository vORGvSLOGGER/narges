...
export default function AvailableOrdersPage() {
  ...
  return (
    <div className="min-h-screen bg-narjis-bg">
      {/* Header */}
      <div className="bg-narjis-green px-4 pt-12 pb-6">
        ...
        <div className={`rounded-2xl p-4 flex items-center justify-between ${driverOnline ? 'bg-white/20' : 'bg-white/10'}`}>
          ...
          <button
            ...
            className={`relative w-14 h-7 rounded-full transition-all duration-300 ${driverOnline ? 'bg-narjis-light' : 'bg-white/30'}`}
          >
      ...
      <div className="px-4 pt-4">
        {!driverOnline ? (
          <div className="text-center py-16">
            ...
            <p className="mt-3 text-narjis-text font-medium">أنت في وضع عدم التوفر</p>
            <p className="text-narjis-text-secondary text-sm">فعّل التوفر لاستقبال الطلبات</p>
          </div>
        ) : pendingOrders.length === 0 ? (
          <div className="text-center py-16">
            <span className="text-6xl">🎉</span>
            <p className="mt-3 text-narjis-text font-medium">لا يوجد طلبات الآن</p>
            <p className="text-narjis-text-secondary text-sm">ستصلك إشعارات عند وصول طلب جديد</p>
          </div>
        ) : (
          <div className="space-y-3">
            <p className="font-bold text-narjis-text">{pendingOrders.length} طلبات تنتظرك</p>
            {pendingOrders.map(order => (
              <div key={order.id} className="card overflow-hidden">
                {/* Order Header */}
                <div className="bg-narjis-bg px-4 py-2 flex items-center justify-between">
                  <span className="font-bold text-narjis-green text-sm">{order.id}</span>
                  <span className="text-xs text-narjis-text-secondary">{formatRelativeTime(order.createdAt)}</span>
                </div>

                <div className="p-4 space-y-3">
                  {/* Customer */}
                  <div className="flex items-center gap-2">
                    <div className="w-10 h-10 bg-narjis-green rounded-xl flex items-center justify-center text-white font-bold">
                      {order.customerName[0]}
                    </div>
                    <div>
                      <p className="font-medium text-sm">{order.customerName}</p>
                      <div className="flex items-center gap-1 text-narjis-text-secondary">
                        <MapPin size={11} />
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="bg-narjis-bg rounded-xl p-2 text-center">
                      <Package size={14} className="mx-auto text-narjis-green mb-1" />
                      <p className="text-xs font-bold">{order.items.length}</p>
                      <p className="text-xs text-narjis-text-secondary">منتج</p>
                    </div>
                    <div className="bg-narjis-bg rounded-xl p-2 text-center">
                      <DollarSign size={14} className="mx-auto text-narjis-orange mb-1" />
                      <p className="text-xs font-bold">{formatSAR(order.deliveryFee)}</p>
                      <p className="text-xs text-narjis-text-secondary">عمولة</p>
                    </div>
                    <div className="bg-narjis-bg rounded-xl p-2 text-center">
                      <MapPin size={14} className="mx-auto text-blue-500 mb-1" />
                      <p className="text-xs font-bold">3.2 كم</p>
                      <p className="text-xs text-narjis-text-secondary">مسافة</p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 bg-narjis-light text-white font-bold py-2.5 rounded-xl active:scale-95 transition-transform"
                    >
                      قبول الطلب ✓
                    </button>
